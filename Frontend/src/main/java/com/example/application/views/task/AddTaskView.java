...
import com.example.application.model.enums.*;
...
public class AddTaskView extends VerticalLayout {
        ...
        if (validateForm()) {
            try {
                ...
                List<Volunteer> selectedVolunteers = volunteerMultiSelectComboBox.getSelectedItems().stream()
                        .map(name -> {
                            // Si se seleccionó la opción automática
                            if (name.equals("Elegir voluntarios automáticamente")) {
                                //Por hacer(Aplicar patron)
                                return new Volunteer("auto", "Automático", "", "");
                            }
                            // Buscar el voluntario por nombre
    ...
    private Component getButtons(){
        ...
        saveTaskButton.addClickListener(e -> {
            saveNewTask();
        });
